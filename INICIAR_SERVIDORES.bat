...
rem Intentar leer rutas guardadas por Instalar.bat
set "FRONTEND_PATH="
...
rem Verificar que la ruta guardada sigue siendo valida
...
rem Si no hay .paths.ini o la ruta guardada es invalida, buscar el frontend
if "%FRONTEND_PATH%"=="" (
    ...
    rem Opcion 1: Frontend en la misma carpeta
    ...
    rem Opcion 2: Frontend al lado (mismo nivel)
    ...
    rem Opcion 3: Buscar en todo el perfil de usuario
    ...
    rem Opcion 4: Buscar por nombre de carpeta
    ...
    if not exist "%FRONTEND_PATH%\index.html" (
        ...
        echo         Ruta que se verifico: %FRONTEND_PATH%
        echo         Asegurate de arrastrar la carpeta que tiene index.html dentro.
        pause
        exit /b 1
    )
)
...
rem Terminal 1: ms-auth (puerto 8001)
...
rem Terminal 2: ms-empleados (puerto 8002)
...
rem Terminal 3: ms-incapacidades (puerto 8003)
...
rem Terminal 4: ms-seguimiento (puerto 8004)
...
rem Terminal 5: frontend (puerto 8080)
